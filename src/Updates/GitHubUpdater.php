...
namespace LoopGridSearch\Updates;
 ...
 *  3. If the remote version is newer than LGS_VERSION the plugin is injected into
 ...
 *  (e.g. loop-grid-search-for-elementor-1.2.3/).  The upgrader_post_install filter
...
final class GitHubUpdater
{
    ...
    private const REPO = 'Magellan-Web-Dev/Loop-Grid-Search-For-Elementor';
    ...
    public static function checkForUpdate(mixed $transient): mixed
    {
        ...
        $current = defined('LGS_VERSION') ? LGS_VERSION : '0.0.0';
    ...
    public static function pluginsApi(mixed $result, string $action, mixed $args): mixed
    {
        ...
        return (object) [
            'name'          => 'Loop Grid Search for Elementor',
            'slug'          => self::PLUGIN_SLUG,
            ...
            'sections'      => [
                'description' => 'AJAX keyword / date / taxonomy search and filtering for any post type, rendered server-side through an Elementor loop template.',
            ],
            'banners' => [],
        ];
    }
    ...
    public static function addActionLinks(array $links): array
    {
        $check_url = wp_nonce_url(
            add_query_arg('action', 'lgs_check_for_updates', self_admin_url('plugins.php')),
            'lgs_check_for_updates',
            'lgs_nonce'
        );
    ...
    public static function handleCheckForUpdates(): void
    {
        if (
            empty($_GET['action']) ||
            $_GET['action'] !== 'lgs_check_for_updates' ||
            empty($_GET['lgs_nonce']) ||
            !wp_verify_nonce(sanitize_text_field(wp_unslash($_GET['lgs_nonce'])), 'lgs_check_for_updates') ||
        ...
        wp_safe_redirect(self_admin_url('plugins.php?lgs_update_checked=1'));
        exit;
    }
    ...
    {
        if (empty($_GET['lgs_update_checked']) || $_GET['lgs_update_checked'] !== '1') {
            return;
        }
        ...
        if ($release && version_compare($release['version'], $current, '>')) {
            $msg = sprintf(
                'Loop Grid Search for Elementor: Version <strong>%s</strong> is available. <a href="%s">Go to Updates</a>.',
                esc_html($release['version']),
                esc_url(self_admin_url('update-core.php'))
            );
        } elseif ($release) {
            $msg = 'Loop Grid Search for Elementor: You are running the latest version.';
        } else {
            $msg = 'Loop Grid Search for Elementor: Could not check for updates. The GitHub repository or release API may be unavailable.';
        }
    ...
    private static function getLatestRelease(): ?array
    {
        ...
        $response = wp_remote_get(self::API_URL, [
            ...
            'headers' => [
                ...
                'User-Agent' => 'WordPress/Loop-Grid-Search',
            ],
        ]);
     ...
     *
     * Example: loop-grid-search-for-elementor/loop-grid-search-for-elementor.php
     *
     * @return string
     */
    ...
    public static function normalizeFolderAfterInstall(mixed $response, mixed $hook_extra, mixed $result): mixed
    {
        ...
        if ($wp_filesystem->is_dir($desired_dir)) {
            $backup_dir = $desired_dir . '_lgs_backup';
